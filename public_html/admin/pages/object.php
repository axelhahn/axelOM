...
$sObjLabel = '' . (isset($aObjdata['icon']) ? icon::get($aObjdata['icon']) : '') . ($aObjdata['label'] ?? '');
            ...
            case 'new':
                if ($o->setItem($aItemData)) {
                    if (!$o->save()) {
                        ...
                        addMsg(
                            'ok',
                            '{{msgok.item_was_created}}: ' . $o->id() . ': ' . $o->getLabel() . ' ' . $sBtnEdit,
                        );
                    }
                } else {
                ...
                if ($sTargetObj && $aTargetIdList) {
                    foreach ($aTargetIdList as $sTargetId) {
                        if ($o->relCreate($sTargetObj, $sTargetId)) {
                            ...
                            addMsg(
                                'error',
                                '{{msgerr.relation_was_not_created}} [' . $sTargetObj . ':' . $sTargetId . ']',
                            );
                        }
                    }
                } else {
            ...
            case 'relDelete':
                ...
                if ($o->read(queryparam::get('id', false, 'int'), true)) {
                    ...
                    if (!$o->relDelete($sRelKey)) {
                        addMsg(
                            'error',
                            '{{msgerr.relation_was_not_deleted}} ['
                            . $sRelKey
                            . '] '
                            . print_r($oDB->lastquery(), 1)
                            . ':-(',
                        );
                    } else {
                    ...
                    foreach ($_FILES as $aFile) {
                        if ($sTargetId = $oFile->uploadFile($aFile)) {
                            ...
                            if ($o->relCreate($sTargetObj, $sTargetId)) {
                                ...
                                http_response_code(503);
                                $out .=
                                    '{{msgerr.relation_was_not_created}} ['
                                    . $sTargetObj
                                    . ':'
                                    . $sTargetId
                                    . ']: '
                            ...
                            http_response_code(503);
                            $out .=
                                '{{msgerr.relation_was_not_created}} upload failed: '
                                . $oDB->error()
                                . print_r($aFile, 1)
                                . PHP_EOL;
                            die($out);
...
if (!$bDbTableOk) {
    addMsg(
        'error',
        '{{object.tablecheck_update_required}}<br>' // .'<hr>' // .'<ul><li>'.implode('</li><li>', $aCheck['_result']['messages']).'</li></ul>'
        . '{{object.tablecheck_edit_disabled}}<br><br>'
        . $renderAdminLTE->getButton([
            'type' => 'dark',
                    ...
                    $acl->canEdit($sTabApp)
                        ? $renderAdminLTE->getButton([
                            ...
                            'onclick' =>
                                ...
                                . $o->getLabel($aRow)
                                . '\n\n?\')) httprequest(\'POST\', location.href , {\'action\': \'delete\', \'id\': \''
                                . $aRow['id']
                                . '\'});',
                        ])
                        : ''
...
if ($bShowEdit && $bDbTableOk) {
    ...
    if (!$acl->canEdit($sTabApp)) {
        ...
        $sForm =
            ''
            . '<form id="frmEditObject" class="form-horizontal" action="'
            . $sBaseUrl
            . '" method="POST">'
        ...
        foreach ($aItem as $sVarname => $value) {
            ...
            if ($bIsEditable) {
                ...
                if($aCfgForm['hooks']??false){
                    foreach($aCfgForm['hooks'] as $sKey=>$sHookMethod) {
                        $aCfgForm[$sKey]= $o->{$sHookMethod}();
                    }
                    unset($aCfgForm['hooks']);
                }
                ...
                if (isset($aCfgForm['tag'])) {
                    switch ($aCfgForm['tag']) {
                        ...
                        case 'select':
                            $sForm .=
                                // '<pre>'.htmlentities($renderAdminLTE->getFormSelect($aCfgForm), 1).'</pre>'.
                                $renderAdminLTE->getFormSelect($aCfgForm);
                            break;
                            ;
                        case 'textarea':
                            $sForm .=
                                // '<pre>'.htmlentities($renderAdminLTE->getFormTextarea($aCfgForm), 1).'</pre>'.
                        ...
                        default:
                            $sForm .=
                                ...
                                . $aCfgForm['tag']
                                . '"<pre>UNHANDLED: '
                                . htmlentities(print_r($aCfgForm, 1), 1)
                                . '</pre>';
                    }
                }
                $sForm .= $sHtmlPost;
            ...
            . (
                $iId
                    ...
                    . $renderAdminLTE->getButton([
                        'type' => 'primary',
                        'onclick' =>
                            '$(\'#frmEditObject\').attr(\'action\', \''
                            . $sBaseUrl
                            . '&id='
                            . $o->id()
                            . '\'); $(\'#frmEditObject\').submit(); return false;',
        ...
        if ($iId) {
            ...
            $sContextbar .=
                ...
                . $renderAdminLTE->getCallout([
                    ...
                    'text' => $aItem['timeupdated']
                        ? '{{last_updated}}:<br>' . $aItem['timeupdated']
                        : '{{created}}:<br>' . $aItem['timecreated'],
                ])
                . $renderAdminLTE->getCallout([
        ...
        if (isset($aItem['id']) && (int) $aItem['id']) {
            if (!$sRelobjectname) {
                ...
                foreach ($appmeta->getObjects() as $sObjname => $aObjdata) {
                    ...
                    $sBtnClass = $bRelationAllowed ? ($sObjname == $sObject ? 'secondary' : 'success') : '';
                ...
                $sContentRelations = $sContentRelations
                    ? icon::get('new') . '{{new}} ... ' . $sContentRelations . '<br><br>'
                    : '';
            } else {
                ...
                if (is_string($oRel)) {
                    ...
                    if ($aRelItems) {
                        $sFormRelations .=
                            '<form method="POST" id="frmAddRel" action="'
                            . $sUrlUp
                            . '#relations">'
                            . '<input type="hidden" name="action" value="relCreate">'
                            . '<input type="hidden" name="relobject" value="'
                            . $o->getTablename($sRelClass)
                            . '">'
                            . '<div class="row mb-3">'
                            . '<label for="selectnewrel" class="col-sm-2 col-form-label">'
                            . icon::get($appmeta->getObjectIcon($sRelobjectname))
                            . $appmeta->getObjectLabel($sRelobjectname)
                            . '<br>' // .'{{select_relation_item}}'
                            . '</label>'
                            . '<div class="col-sm-10">'
                        ...
                        if ($aRelItems && count($aRelItems)) {
                            ...
                            foreach ($aRelItems as $aItem) {
                                ...
                                $sOptionLabel =
                                    $oRel->getLabel($aItem) . ($sHintForOption ? ' <<< ' . $sHintForOption . '' : '');
                    ...
                    $sBtnDel = $aRelation['_column']
                        ...
                        : $renderAdminLTE->getButton([
                            ...
                            'onclick' =>
                                ...
                                . ' -> '
                                . $sTargetLabel
                                . '\n\n?\')) httprequest(\'POST\', location.href , {\'action\': \'relDelete\', \'relkey\': \''
                                . $sRelKey
                                . '\'});',
                        ]);
...
$BODY = $renderAdminLTE->addRow(
    $renderAdminLTE->addCol(renderMsg() . $sMainContent, 10) . $renderAdminLTE->addCol($sContextbar, 2),
);
